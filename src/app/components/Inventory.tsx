...
export function Inventory() {
  ...
  const handleRestock = async (item: InventoryItem) => {
    const restockQuantity = prompt(`Enter quantity to add to ${item.name}:`, '10');
    if (restockQuantity && !isNaN(parseFloat(restockQuantity))) {
      try {
        await api.post(`/inventory/${item.id}/restock`, { quantity: parseFloat(restockQuantity) });
        await fetchInventory();
      } catch (error) {
